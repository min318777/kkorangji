...
### 조회수 처리 방식 비교
매 조회마다 DB UPDATE 시 트래픽 몰릴 때 병목 발생. k6 부하 테스트에서 동시 요청 시 조회수의 약 80%가 유실되는 것을 확인하고, 두 가지 방식을 비교

- v1: 더티체킹 — JPA 자동 UPDATE, 동시 요청 시 Lost Update 발생 (Read-Modify-Write 사이 경쟁 상태로 증가분이 덮어써짐)
- v2: 원자적 UPDATE — `SET view = view + 1` 쿼리 한 번으로 처리, 동시성 문제 없음

v2 채택. DB가 증가 연산 자체를 원자적으로 수행하므로 Lost Update가 발생하지 않으며, 별도 인프라(Redis 배치 동기화, 비관적 락) 없이도 단순한 구조로 정확한 조회수를 보장
...
### 게시글 검색 방식 개선
LIKE 검색은 검색어 앞뒤에 와일드카드를 붙이는 방식이라 인덱스가 정렬해둔 순서로 값을 찾아 들어갈 수 없어 인덱스를 타지 못하고 전체 테이블을 처음부터 끝까지 읽음. 데이터 100만 건 기준 4,500ms 응답 지연 확인

- Full-Text Search 인덱스 도입, 문법 분석 없이도 한글 부분 검색이 가능하도록 2글자 단위로 쪼개어 색인하는 ngram 파서 적용
- 실행 계획 분석 결과 LIKE는 인덱스 후보 자체가 없어 조건 검사를 매 행마다 반복하는 반면, Full-Text Search는 검색어와 일치하는 문서만 인덱스로 즉시 찾아냄

데이터 100만 건 기준 응답 속도 4,500ms → 1,300ms로 단축
